...
describe('Room API', () => {
  ...
  describe('POST /room/:roomId/reveal', () => {
    it('reveals votes, broadcasts, and returns gameEvent', async () => {
      ...
      await request(app)
        .post('/room/123/reveal')
        .expect(200)
        .expect({ success: true, gameEvent: fakeEvent });
  ...
  describe('POST /room/:roomId/repeat', () => {
    it('repeats voting, broadcasts, and returns item', async () => {
      ...
      await request(app)
        .post('/room/321/repeat')
        .expect(200)
        .expect({ success: true, item: 'A' });
  ...
  describe('POST /room/:roomId/summary', () => {
    ...
    it('shows summary, broadcasts, schedules delete, and returns gameEvent', async () => {
      ...
      const bc = vi
        ...
        .mockImplementation(() => {});
      const del = vi
        .spyOn(roomService, 'deleteRoom')
        .mockImplementation(() => {});

      await request(app)
        .post('/room/999/summary')
        .expect(200)
        .expect({
          success: true,
